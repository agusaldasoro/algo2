  ...
  public:
    ...
    class Iterador
    {
      public:
        ...
        ///Pre: clave no pertenece a las claves
        void AgregarComoSiguiente(const K& clave, const S& significado); ///Modificado
        void AgregarComoAnterior(const K& clave, const S& significado); ///Modificado
...
{
  it_claves_.AgregarComoSiguiente(clave);
  it_significados_.AgregarComoSiguiente(significado);
}
...
{
  it_claves_.AgregarComoAnterior(clave);
  it_significados_.AgregarComoAnterior(significado);
}
...
typename Dicc<K,S>::const_Iterador Dicc<K,S>::Buscar(const K& clave) const
{
  ...
  while(it.HaySiguiente() && not(it.SiguienteClave() == clave)){
      it.Avanzar();
  }
  return it;
}
